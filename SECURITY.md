...
itself, not inside a Spark driver. `LIGHTER_PY_GATEWAY_AUTH_TOKEN` requires a shared secret
from clients of that port; it is unset by default, and Lighter logs a warning at startup while
it is.

Network access to Lighter is therefore equivalent to shell access on the Spark cluster, and
access to the gateway port is equivalent to shell access on the Lighter host unless a token is
set. Both ports must be reachable only from networks and clients you trust. See
